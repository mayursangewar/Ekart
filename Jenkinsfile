pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/mayursangewar/Ekart.git'
            }
        }
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t mayursangewar/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker push mayursangewar/ekart:latest'
                }
            }
        }
